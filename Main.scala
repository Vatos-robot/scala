...
object Main extends App {
  ...
  /** Read action from Stdin and execute it Exit if action is 'exit' or if an
    * error occured (status > 0) DO NOT MODIFY THIS FUNCTION
    */
  def run(canvas: Canvas = Canvas()): Status = {
    ...
    val (newCanvas, status) =
      execute(ArraySeq.unsafeWrapArray(action.split(' ')), canvas)
  ...
  /** Execute various actions depending on an action command and optionnaly a
    * Canvas
    */
  def execute(action: Seq[String], canvas: Canvas): (Canvas, Status) = {
    val execution: (Seq[String], Canvas) => (Canvas, Status) =
      action.head match {
...
case class Canvas(
  ...
  /** Takes a pixel in argument and put it in the canvas in the right position
    * with its color
    */
  ...
  def dummy(arguments: Seq[String], canvas: Canvas): (Canvas, Status) =
    if (arguments.size > 0)
      (
        ...
        Status(
          error = true,
          message = "dummy action does not excpect arguments"
        )
      )
    else {
